...
import { getProjectByShareToken, recordShareView } from '@/lib/shareLinks';
...
export function SharedProjectPage() {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      try {
        let projectData: any;
        let shareTokenToUse: string | null = null;

        // Handle permalink (short URL with token)
        if (token) {
          const result = await getProjectByShareToken(token);
          projectData = result.project;
          shareTokenToUse = result.shareToken;
          setShareToken(shareTokenToUse);
        } 
        // Handle direct project ID access
        else if (id) {
        ...
        // Record view
        await recordShareView(projectData.id, shareTokenToUse || undefined);
      } catch (error) {
